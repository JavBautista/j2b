...
/**
 * API (app móvil): cuenta corriente del cliente (saldo a favor).
 * Espejo de App\Http\Controllers\Admin\ClientAccountController (web), pero por token JWT.
...
class ClientAccountController extends Controller
{
    ...
    {
        $this->authorizeShop($request, $client);
    ...
    /** El cliente debe pertenecer a la tienda del usuario autenticado (token). */
    private function authorizeShop(Request $request, Client $client): void
    {
        if ($client->shop_id !== $request->user()->shop->id) {
            abort(404);
        }
    }
}
